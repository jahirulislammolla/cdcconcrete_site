...
@section('contents')
                                    ...
                                    @foreach ($all as $key => $item)
                                        ...
                                        <tr class="text-center">
                                            ...
                                            @isset($item->link)
                                                ...
                                                <a href="{{ $item->link }}" target="_blank" rel="nofollow" alt='nofollow'>{{ $item->name }}</a></td>
                                                @else
                                                <a href="http://{{ $item->link }}" target="_blank" rel="nofollow" alt='nofollow'>{{ $item->name }}</a></td>
                                                @endif
                                            @endisset
                                            <td>
                                                ...
                                                <a onclick="return confirm('Are you sure?')" href="{{ route('editor_client_information_delete',['id'=>$item->id]) }}" class="btn btn-danger btn-sm m-1"><i class="fa fa-trash"></i> Delete</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
        ...
        <div class="modal-dialog modal-lg modal-center" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalCreateLabel">Add Client Information</h5>
                ...
                <div class="modal-body">
                    <!-- Modal body -->
                    <form id="contact_create_form" action="{{ route('editor_client_information_create') }}" enctype="multipart/form-data" method="POST">
                        @csrf
                        <div class="modal-body">
                            <div class="form-group mb-3">
                                <label for="name">Name:</label>
                                <input type="text"  id="name" name="name" placeholder="client name" class="form-control input_val"  required/>
                            </div>
 
                            <div class="form-group mb-3">
                                <label for="image">Image(225px * 225px):</label>
    ...
    <div class="modal fade" id="exampleModalUpdate" tabindex="-1" aria-labelledby="exampleModalUpdateLabel" style="display: none;" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-center" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalUpdateLabel">Update Client Information</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true"></span>
                    </button>
                </div>
                <div class="modal-body">
                    <!-- Modal body -->
                    <form id="contact_update_form" action="{{ route('editor_client_information_update') }}" enctype="multipart/form-data" method="POST">
                        @csrf
                        <div class="modal-body">
                            <div class="form-group mb-3">
                                <label for="name">Name:</label>
